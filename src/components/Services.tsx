...
export default function Services() {
  return (
    <section id="services" className="py-24 bg-gradient-to-br from-pink-50 via-white to-emerald-50 relative overflow-hidden">
      <div className="absolute inset-0 opacity-5">
        <svg className="w-full h-full" viewBox="0 0 800 800">
          <circle cx="400" cy="400" r="350" fill="none" stroke="rgb(217, 119, 6)" strokeWidth="1" strokeDasharray="10,10" />
        </svg>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {services.map((service, index) => {
            ...
            const bgColor = service.color === 'pink' ? 'bg-pink-50' : service.color === 'amber' ? 'bg-amber-50' : 'bg-emerald-50';
            const iconColor = service.color === 'pink' ? 'text-pink-400' : service.color === 'amber' ? 'text-amber-600' : 'text-emerald-700';
            const borderColor = service.color === 'pink' ? 'border-pink-200' : service.color === 'amber' ? 'border-amber-200' : 'border-emerald-200';

            return (
              <div
                key={index}
                className={`${bgColor} rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border-2 ${borderColor} transform hover:-translate-y-2`}
              >
                <div className="mb-6">
                ...
                <a
                  href="#contact"
                  className="block text-center bg-white text-gray-800 px-6 py-3 rounded-full hover:bg-gray-50 transition-colors duration-200 font-medium shadow-md border-2 border-gray-200"
                >
                  Learn More
                </a>
              </div>
        ...
        <div className="mt-16 bg-gradient-to-r from-pink-100 via-amber-100 to-emerald-100 rounded-2xl p-8 text-center shadow-lg border-2 border-amber-200">
          <h3 className="text-2xl font-serif text-gray-800 mb-4">Not sure which service is right for you?</h3>
